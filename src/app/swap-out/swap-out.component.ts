...
import { interval, tap, take, switchMap, first, Observable } from 'rxjs';
import {
  Chain,
  Asset,
  TransferWrapper,
...
@Component({
  ...
  templateUrl: './swap-out.component.html',
  styleUrl: './swap-out.component.scss',
})
export class SwapOutComponent implements OnInit {
  ...
  set searchText(val: string) {
    ...
    if (this.selectedSourceDenom) {
      ...
      (this.selectedExitChain && !this.selectedExitDenom)
    ) {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.skip_service.getChains().subscribe((res) => {
      ...
      //this.selectedOriginChain = res.chains[0];
  ...
  removeSelectedChain() {
    ...
    this.selectedExitChain = undefined;
  ...
  removeSelectedSourceDenom() {
    ...
    this.resetInputs();
    this.resetChecks();
    this.getAvailableDenoms(this.selectedOriginChain?.chain_id);
  }
